            ...
            for mon_path in monitor_dirs:
                ...
                if self._enabled:
                    self._scheduler.add_job(func=self.copy_files, trigger='date',
                                            ...
                                            name=f"文件移动 {mon_path}")
            # 运行一次定时服务
            if self._onlyonce:
                logger.info("文件移动服务启动，立即运行一次")
                self._scheduler.add_job(name="文件移动", func=self.copy_files, trigger='date',
                                        run_date=datetime.datetime.now(
                                            tz=pytz.timezone(settings.TZ)) + datetime.timedelta(seconds=3)
                                        )
                # 关闭一次性开关
        ...
        for mon_path in self._dirconf.keys():
            ...
            for file in files:
                ...
                if Path(cloud_file).exists():
                    ...
                    continue
                state, error = SystemUtils.move(file, Path(cloud_file))
